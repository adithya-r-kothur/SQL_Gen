...
def genai():
    ...
    if(text_from_image is None):
        ...
        prompt ='''Hello Adithya'''
        if(len(text_from_image)>0):
            response = model_text.generate_content(prompt+text_from_image + sql_query)
...
st.set_page_config(page_title ="SQL_Gen", initial_sidebar_state = "expanded")
